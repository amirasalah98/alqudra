...
if(isset($_REQUEST['del_mage'])&&$_REQUEST['del_mage']!='' )
{
  ...
  $file_path     = '../qudrahimg/alqudrah_products/'.$del_mage;
  $operation->unlinkfile($file_path);
				  
  $file_path     = '../qudrahimg/alqudrah_products/thump/'.$del_mage;
  $operation->unlinkfile($file_path);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
 ...
 <tr>
    <td width="35%" class="tdstyl_r"><b>&nbsp;&nbsp;Category</b></td>
    <td width="29%" class="tdstyl_r">&nbsp;&nbsp;
      <select name="cat_id"  onchange="get_subcat2(this.value,<?=$products['sub_id']?>)">
        <? 
	...
	?>
        ...
        <option value="<?=$id?>" <? if($id==$products['cat_id']){ echo 'selected="selected"';} ?> >
        <?=stripslashes($name)?>
        </option>
        <? }?>
      </select></td>
  ...
  <tr>
    <td width="35%" class="tdstyl_r"><b>&nbsp;&nbsp;SubCategory</b></td>
    <td width="29%" class="tdstyl_r">&nbsp;&nbsp;
      <select name="scat_id" id="sub_cath" >
        <? 
	$ordr='order by id desc';
	$categories=$operation->getdata('alqudrah_subcategories','*','cat_id',"'".$products['cat_id']."'",'','','');
	?>
        <option value=""  selected="selected">Select One </option>
        <? foreach($categories as $key => $val ){ extract($val);?>
        <option value="<?=$id?>" <? if($id==$products['sub_id']){ echo 'selected="selected"';} ?> >
        <?=stripslashes($name)?>
        </option>
        <? }?>
      </select>
   ...
   <tr>
    ...
    <td class="tdstyl_r">&nbsp;&nbsp;<input type="text" name="name" value="<?=stripslashes($name)?>"/></td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
  <tr>
    <td class="tdstyl_r"><b>&nbsp;Item Id</b></td>
    <td class="tdstyl_r">&nbsp;&nbsp;<input type="text" name="itemid" value="<?=stripslashes($itemid)?>" /></td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
   <tr>
    <td class="tdstyl_r"><b> Manufacturer</b></td>
    <td class="tdstyl_r">&nbsp;&nbsp;<input type="text" name="m_name" value="<?=stripslashes($manufacturer)?>"/></td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
   <tr>
    <td class="tdstyl_r"><b> Type</b></td>
    <td class="tdstyl_r">&nbsp;&nbsp;<input type="text" name="type" value="<?=stripslashes($type)?>"/></td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
   <tr>
    <td class="tdstyl_r"><b> Serial</b></td>
    <td class="tdstyl_r">&nbsp;&nbsp;<input type="text" name="serial" value="<?=stripslashes($serial)?>"/></td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
   <tr>
    <td class="tdstyl_r"><b> Year</b></td>
    <td class="tdstyl_r">&nbsp;&nbsp;<input type="text" name="year" value="<?=stripslashes($year)?>"/></td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
   <tr>
    ...
    <td class="tdstyl_r">&nbsp;&nbsp;&nbsp;&nbsp;
      <?
					...
					$oFCKeditor1->Value		= stripslashes($details);
					$oFCKeditor1->Create() ;
				?>      </td>
    <td class="tdstyl_r">&nbsp;</td>
  </tr>
  <tr>
    ...
    <td colspan="2" class="tdstyl_r">&nbsp;&nbsp;<img src="../qudrahimg/alqudrah_products/thump/<?=$image?>" />
...
{
 document.getElementById("h_subcid").value = str;
}
</script>
