...
{
 extract($_GET);
 $alqudrah_images=$operation->getdata_1('alqudrah_products','*','',''," id=$del_id");
 $delete=$operation->delete('alqudrah_products','id',$del_id);
 $operation->unlinkfile("../qudrahimg/alqudrah_products/".$alqudrah_images['image']);
 $operation->unlinkfile("../qudrahimg/alqudrah_products/thump/".$alqudrah_images['image']);
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  ...
  <tr>
    <td width="16%" class="tdstyl_r">&nbsp;Select Item ID</td>
    <td width="20%" class="tdstyl_r">&nbsp;&nbsp;&nbsp;
			   
      <select name="item_id">
        <? 
	
	$products=$operation->getdata('alqudrah_products','*','','','','','','');
	?>
        <option value="0" >Select One </option>
        <? foreach($products as $productskey => $productsval ){ ?>
        <option value="<?=stripslashes($productsval['itemid'])?>" <? if(stripslashes($productsval['itemid'])==$item_id) { echo "selected='selected'";}?>  >
          <?=stripslashes($productsval['itemid'])?>
          </option>
        <? }?>
      </select></td>
    <td width="15%" class="tdstyl_r">&nbsp;</td>
    ...
    <td colspan="5" class="tdstyl_r">&nbsp;</td>
            ...
            <tr  >
              <td width="29" height="28" align="center" valign="middle" class="right_tile"  style="color:#FFFFFF" colspan="3">Name </td>
			   <td width="10%" align="center" valign="middle" class="right_tile" style="color:#FFFFFF">Item ID</td>
			     <td width="12%" align="center" valign="middle" class="right_tile" style="color:#FFFFFF">&nbsp;</td>
		...
		<? if(count($date)>0){ foreach($date as $key=>$val){ extract($val);?>	
            <tr>
              <td align="left" valign="middle" height="30"  class="tdstyl_r" colspan="3"><div style="padding-left:5px;"><?=stripslashes($name)?></div></td>
			  <td align="left" valign="top" class="tdstyl_r"><?=stripslashes($itemid)?></td>
			  ...
			  <td align="left" valign="top" class="tdstyl_r" >&nbsp;<a href="products.php?fn=del&amp;del_id=<?=$id?>"><img src="images/DeleteRed.png"  border="0" onclick="return confirm('Are you sure you want to delete?')" /></a></td>
            </tr>
			<? } }?>
			<tr>
